...
if($_SERVER['REQUEST_METHOD'] === 'POST'){
    ...
    if(empty($errors)){
        ...
        $stmt = $conn->prepare("SELECT status, price_per_night, room_type, room_number FROM rooms WHERE room_id = ?");
        ...
        if(!$room){
            ...
            if($stmt->execute()){
                ...
                // Store booking_id and email in session for payments.php
                $_SESSION['booking_id'] = $booking_id;
                $_SESSION['customer_email'] = $email; // <-- key must match payments.php
                ...
                echo "<a href='payments.php' style='display:inline-block;margin-top:20px;padding:10px 20px;background:#28a745;color:white;border-radius:5px;text-decoration:none;'>View Payments</a><br>";
                echo "<a href='customer_portal.php' style='display:inline-block;margin-top:10px;padding:10px 20px;background:#007bff;color:white;border-radius:5px;text-decoration:none;'>Back to Customer Portal</a>";
                echo "</div>";
